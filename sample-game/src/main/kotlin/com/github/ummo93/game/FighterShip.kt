...
class FighterShip : TexturedActor, Controllable, Damagable {
    ...
    var velocity: Vector2 = vector2Zero()
    ...
    override fun onUpdate(dt: Float) {
        ...
        val newTranslatedPos = RaylibUtils.translate2D(position, vector2Scale(velocity, dt * 75f * speed), 1f)
    ...
    override fun onDestroy() {
        val explosionPoint = vector2SubtractValue(position, explosionAnimation!!.frameWidth / 2f)
    ...
    protected fun drawBullets() {
        for (bullet in bullets) {
            drawLineEx(
                bullet.pos,
                vector2Add(bullet.pos, vector2Scale(bullet.fwd, 8f)),
                2f,
                Colors.RED
            )
        }
    ...
    protected fun refreshBulletsState(dt: Float) {
        ...
        for (bullet in bullets) {
            bullet.pos = RaylibUtils.translate2D(bullet.pos, vector2Scale(bullet.fwd, dt * 800f), 1f)
    ...
    private fun drawHeadingVector() {
        ...
        drawText("<>", (endPos.x()).toInt() - 5, (endPos.y()).toInt() - 5, 14, Colors.DARKGREEN)
    }

    private fun drawForwardExhaust(origin: Vector2, fwd: Vector2) {
        val backward = vector2Negate(fwd)
        val endPosBezier = RaylibUtils.translate2D(origin, backward, getRandomValue(15, 25).toFloat())
        val endPosBezier2 = RaylibUtils.translate2D(origin, backward, getRandomValue(20, 30).toFloat())
        drawLineEx(endPosBezier, origin, 6f, RaylibUtils.color(112, 31, 126, 150))
        drawLineEx(endPosBezier2, origin, 2f, RaylibUtils.color(0, 121, 241, 200))
    }

    private fun drawBackwardExhaust(origin: Vector2, fwd: Vector2) {
        val endPosBezier = RaylibUtils.translate2D(origin, fwd, getRandomValue(8, 13).toFloat())
        val endPosBezier2 = RaylibUtils.translate2D(origin, fwd, 13f)
        drawLineEx(endPosBezier, origin, 18f, RaylibUtils.color(0, 121, 241, 150))
        drawLineEx(endPosBezier2, origin, 14f, Colors.BLACK)
    }
        ...
        if (controlSignals.contains(ControlSignal2D.ROTATE_COUNTERCLOCKWISE)) {
            ...
            if (abs(rotationVelocity.toDouble()) > 0.05f) {
                rotationVelocity = if (rotationVelocity > 0) {
                    clamp(rotationVelocity - 0.1f, -rotationMaxSpeed, rotationMaxSpeed)
                } else {
                    clamp(rotationVelocity + 0.1f, -rotationMaxSpeed, rotationMaxSpeed)
                }
            }
    ...
    private fun calculateVelocity(): Vector2 {
        ...
        if (controlSignals.contains(ControlSignal2D.MOVE_FORWARD)) {
            ...
            isForwardEngineActive = false
            isBackwardEngineActive = false
            newVelocity = if (vector2Length(newVelocity) > 0.25f) {
                vector2Scale(newVelocity, 0.996f)
            } else {
